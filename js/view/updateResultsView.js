...
function updateResultsView(results) {
	...
	document.querySelector('#total-month-payment').innerHTML = priceFormatterDecimals.format(
		results.monthPayment,
	);
	document.querySelector('#total-cost').innerHTML = priceFormatter.format(results.totalAmount);
	document.querySelector('#total-overpayment').innerHTML = priceFormatter.format(
		results.overPayment,
	);
}
